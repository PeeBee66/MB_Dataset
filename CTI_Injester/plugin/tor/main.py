...
def ingest_to_opencti(only_new=True):
    """Manual ingest of TOR nodes to OpenCTI - process each line individually"""
    config = load_config()

    try:
        # Choose file based on only_new parameter
        if only_new:
            # Use NEW_NODES.csv for only new nodes
            if not NEW_NODES_FILE.exists():
                return {'success': False, 'error': 'No new TOR nodes available. Please fetch TOR nodes first.'}

            df = pd.read_csv(NEW_NODES_FILE)
            file_source = "NEW_NODES.csv"
        else:
            # Use the latest TOR nodes CSV file for all nodes
            ...
            file_source = "tor_nodes_latest.csv"
        ...
        logger.info(f"Manual ingest: Processing {len(df)} TOR nodes from {file_source}")
...
def create_blueprint():
    ...
    def ingest():
        """Manually trigger OpenCTI ingestion - ingests ALL nodes"""
        result = ingest_to_opencti(only_new=False)
        return jsonify(result)

    @bp.route('/ingest_to_opencti', methods=['POST'])
    def ingest_to_opencti_api():
        """API route for template to trigger ingestion - ingests ALL nodes"""
        result = ingest_to_opencti(only_new=False)
        return jsonify(result)
